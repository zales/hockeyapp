module HockeyApp
  class Version
    ...
    ATTRIBUTES = %i[id notes shortversion version mandatory timestamp appsize title download_url].freeze

    POST_PAYLOAD = %i[status ipa dsym notes_type notify tags].freeze
    ...
    def crashes
      @crashes ||= @app.crashes.select { |crash| crash.app_version_id.to_s == @id.to_s }
    end
